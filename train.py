...
def train(model, dataloader, criterion_cls, criterion_reg, optimizer, device):
    ...
    for batch in dataloader:
        ...
        gt_segments = actions[:, :, 1:]  # Shape: (batch_size, max_proposals, 2) in seconds

        # Normalize ground truth segments to feature space (0 to max_length)
        batch_size = video.size(0)
        max_length = video.size(1)  # 300
        scale_factors = max_length / durations.unsqueeze(1).unsqueeze(2)  # Shape: (batch_size, 1, 1)
        gt_segments_normalized = gt_segments * scale_factors  # Scale seconds to feature indices

        # Loss: Classification + Regression
        valid_mask = gt_classes != -1  # Mask for valid actions
